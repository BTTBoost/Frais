...
contract CourseContract {
    ...
    function addMaintainer(address newMaintainer, uint256 amount)
        public
        restricted
    {
    ...
    function voteRequest(uint256 ID) public restricted {
    ...
    function approveRequest(uint256 ID) public restricted {
    ...
    function setAllowance(address sender, uint256 _amount) public hasTokens {
    ...
    function updateAllowances(uint256 _amount) public {
        for (uint256 i = 0; i < tokenHoldersCounter; i++) {
            allowance[tokenHolders[i]] +=
                (balance[tokenHolders[i]] / (1000 - tokenPot)) *
                _amount;
        }
    }

    function setCoursePrice(uint256 price) public restricted {
        //maintainers can change the price of the course at any time
        coursePrice = price;
    }

    function transferToken(address _To, uint256 _amount) public hasTokens {
    ...
    function transferTokenPot(address _To, uint256 _amount) public restricted {
    ...
    function withdraw() external payable isEnrolled {
        uint256 banked = allowance[msg.sender];
        require(banked > 0);
