...
contract CourseContract {
    ...
    */
    ...
    mapping(uint256 => Module[]) moduleVersion;
    mapping(uint256 => Module[]) moduleKey;
    mapping(uint256 => Edit[]) editKey;
    ...
    function createRequest(
        ...
        uint256[] memory _targetID,
        string[] memory _qHash,
        string[] memory _mHash
    ) public {
        author = msg.sender;

        for (uint256 i = 0; i < _nameMod.length; i++) {
            moduleToRequest(
                _nameMod[i],
                _descriptionMod[i],
                _mHashMod[i],
        ...
        for (uint256 i = 0; i < _qHash.length; i++) {
    ...
    function returnRequest(uint256 ID)
        public
        view
        returns (
            string memory name,
        ...
        )
    {
        ...
        for (uint256 i = 0; i < modulesArray.length; i++) {
    ...
    function returnEdit(uint256 _ID)
        public
        view
        returns (
            uint256[] memory ID,
    ...
    function approveRequest(uint256 _index) public restricted {
    ...
    function approveModules(uint256 ID) public restricted {
        Module[] memory modulesArray = moduleKey[ID];
    ...
    function approveEdits(uint256 ID) public restricted {
